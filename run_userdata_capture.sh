...
# Set variables
PROJECT_DIR="$(pwd)"
mkdir -p "$PROJECT_DIR/logs_v2"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="$PROJECT_DIR/logs_v2/mellowtel_userdata_capture_${TIMESTAMP}.log"

# Parse command-line arguments
export ENABLE_TC=false
export ENABLE_RATE_LIMIT=false
export EXPERIMENT_TYPE=userdata
...
# Log start
echo "========================================" >> "$LOG_FILE"
echo "$(date): Starting Mellowtel user data capture experiment (23 hours)" >> "$LOG_FILE"
...
# Run docker-compose
echo "$(date): Starting new container for user data capture experiment..." >> "$LOG_FILE"
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "$(date): Mellowtel user data capture experiment completed successfully" >> "$LOG_FILE"
else
    echo "$(date): ERROR - Mellowtel user data capture experiment failed with exit code $EXIT_CODE" >> "$LOG_FILE"
fi
