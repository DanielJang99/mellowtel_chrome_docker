#!/usr/bin/env python3
"""
Mellowtel SDK Network Analysis Tool - 23-Hour User Data Capture Mode
Captures all network activity from Chrome browsing with SupportWithMellowtel extension.
Visits https://yasirzaki.net for 23 hours and saves user-data directory as zip archive.
"""
...
class NetworkAnalyzer:
    """Main class for running the 23-hour user data capture network analysis experiment."""
    ...
    def __init__(self):
        ...
        # Use only SupportWithMellowtel.crx extension
        self.extension_name = 'SupportWithMellowtel.crx'
        self.extension_path = os.path.join('crx_files', self.extension_name)
        logger.info(f"Using extension: {self.extension_name}")
        ...
        self.driver = None
        self.user_data_dir = None  # Store user data directory path for compression
    ...
    def get_extension_id(self) -> str:
        """
        Get the extension ID for SupportWithMellowtel extension.
        ...
        try:
            ...
            # Get all extension IDs and find SupportWithMellowtel
            script = """
            ...
            for ext in extensions:
                ...
                # Look for SupportWithMellowtel
                if 'supportwithmellowtel' in ext['name'].lower():
            ...
            if extensions and len(extensions) > 0:
                ...
                logger.warning(f"Could not find 'SupportWithMellowtel' by name. Using first extension: {self.extension_id}")
    ...
    def activate_extension(self):
        """
        Activate the SupportWithMellowtel extension by navigating to its popup URL.
        No interaction needed - just wait for popup to appear.
        """
    ...
    def compress_user_data(self) -> str:
        """
        Compress the user-data directory and save to output folder.
        Returns the path to the created zip file.
        """
        try:
            if not self.user_data_dir or not os.path.exists(self.user_data_dir):
                logger.warning("User data directory not available for compression")
                return None

            logger.info(f"Compressing user data directory: {self.user_data_dir}")

            # Generate timestamp for zip filename
            timestamp = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
            zip_filename = f'chrome_profile_{timestamp}.zip'
            zip_path = os.path.join(self.run_dir, zip_filename)

            # Ensure output directory exists
            Path(self.run_dir).mkdir(parents=True, exist_ok=True)

            # Create zip archive with all user-data contents
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for root, dirs, files in os.walk(self.user_data_dir):
                    for file in files:
                        file_path = os.path.join(root, file)
                        # Create archive name relative to user_data_dir
            ...
            logger.info(f"[SUCCESS]User data compressed to: {zip_filename} ({zip_size_mb:.2f} MB)")

            return zip_path

        except Exception as e:
            logger.error(f"Failed to compress user data: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _process_site_after_navigation(self, url: str):
        ...
        try:
            ...
            # Activate the extension only once (before starting monitoring)
            ...
            if total_iframes_found > 0:
                ...
                logger.warning(f"No Mellowtel iframes detected after {self.monitoring_duration} seconds")
    ...
    def run_experiment(self):
        ...
        logger.info("=" * 70)
        logger.info("Mellowtel SDK Network Analysis Tool - User Data Capture Mode")
        logger.info("=" * 70)
        ...
        logger.info(f"    - User data archive: chrome_profile_<timestamp>.zip")
        ...
        logger.info(f"  - Compressing user data directory before exit")
        logger.info("=" * 70)

        # Get hardcoded target site
        site = self.get_target_url()
        ...
        try:
            ...
            # Compress user data directory before quitting driver
            if self.user_data_dir and os.path.exists(self.user_data_dir):
                zip_path = self.compress_user_data()
                if zip_path:
                    logger.info(f"  - User data archive: {os.path.basename(zip_path)}")
                else:
                    logger.warning("  - User data archive: compression failed")
